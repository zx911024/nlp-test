...
flags.DEFINE_string("errorlog_file",     "error.log",    "File for log")
flags.DEFINE_string("map_file",     "maps1.pkl",     "file for maps")
...
def train():
    # load data sets
    # train_sentences [[[u'word',u'tag'],[u'word',u'tag']]]
    ...
    errorlog_path=os.path.join("log",FLAGS.errorlog_file)
    errorlogger = get_logger(errorlog_path)
        ...
        for i in range(100):
            for batch in train_manager.iter_batch(shuffle=True):
                ...
                if step % FLAGS.steps_check == 0:
                    ...
                    logger.info("iteration:{} step:{}/{}, ""NER loss:{:>9.6f}".format(iteration, step%steps_per_epoch, steps_per_epoch, np.mean(loss)))
                    loss = []
...
def evaluate_line():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        line=u"律师解读郭敬明性骚扰事件:若无证据 对李枫不利"
        #line=content
        result = model.evaluate_line(sess, input_from_line(line, char_to_id), id_to_tag)
...
class Infer:
    ...
    def creat_model(self):
        ...
        #    self.model = create_model(self.sess, Model, FLAGS.ckpt_path, load_word2vec, config, self.id_to_char, logger)
    ...
    def infer(self, input_str):
        ...
        tf_config.gpu_options.allow_growth = True
        #print self.sess
        result = self.model.evaluate_line(self.sess, input_from_line(input_str, self.char_to_id), self.id_to_tag)
        return result

#if __name__=="__main__":
    #inf = Infer()
    #print inf.infer(u"律师解读郭敬明性骚扰事件:若无证据 对李枫不利")
    #print main()
